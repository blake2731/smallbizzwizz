...
export async function GET() {
  ...
  if (!auction) return Response.json({ error: 'No auction found' }, { status: 404 })
  ...
  if (!buyer || buyer.auctionId !== auction.id) {
    return Response.json({ error: 'Gerry Saunders not found' }, { status: 404 })
  }
  ...
  const totalOunces = 27

  await db
    .update(auctionPackage)
    .set({
      weightOunces: totalOunces,
      lengthHundredths: 1800,
      widthHundredths: 1400,
      heightHundredths: 300,
      status: 'packed',
      shippingCents: null,
  ...
  await db
    ...
    .set({
      ...
      packageLengthHundredths: 1800,
      packageWidthHundredths: 1400,
      packageHeightHundredths: 300,
      updatedAt: new Date(),
    })
